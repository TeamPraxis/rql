...
const parse = (/*String|Object*/query, parameters) => {
	...
	const leftoverCharacters = query.replace(/(\))|([&|,])?([+*$\-:\w%._]*)(\(?)/g,
		(t, closedParan, delim, propertyOrValue, openParan) => {
			if (delim) {
				if (delim === '&') {
					setConjunction('and');
				}
...
exports.parseGently = parseGently;